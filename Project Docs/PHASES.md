...
**Status: In progress — codebase deploy-readiness complete, account-level deployment not yet done.** A second hosting option the owner found (`onehostmarket.com`, a budget cPanel reseller) was evaluated 2026-09-03 and set aside — no docs confirming Python/WSGI or PostgreSQL support, and a paid plan rather than free — so the plan stays Render + Neon + UptimeRobot as originally decided. The codebase itself was made deploy-ready the same day: `gunicorn` (WSGI server), `whitenoise` (static files, production-only manifest storage), and media storage against **Cloudinary** (Render's free tier has no persistent disk, confirmed directly, so uploaded media like the Resume PDF and every `ProjectImage` screenshot would otherwise be wiped on each redeploy) — Cloudflare R2 was tried first but requires a card on file to activate at all (confirmed live in Cloudflare's own dashboard), so this switched same-day to Cloudinary, whose free tier needs no card. A dedicated Cloudinary API key was created and its real connectivity verified with an actual upload/delete round-trip (checked via Cloudinary's Admin API, not just the Django-level check), plus production security settings (`CSRF_TRUSTED_ORIGINS`, `SECURE_PROXY_SSL_HEADER`, HSTS, cookie-secure flags) and a `render.yaml` Blueprint defining the service end-to-end. Verified via `manage.py check` and a simulated production `manage.py check --deploy` (both clean bar the pre-existing intentional `X_FRAME_OPTIONS` warning) and a real `collectstatic` run against the Tailwind-built static tree. See `DECISIONS.md` for full detail. **Not yet done** (all account-level, need the owner's own credentials — see `TASKS.md`): creating the Render service from the Blueprint, pasting the Neon `DATABASE_URL` and Cloudinary env vars into Render, the first live deploy, and the UptimeRobot monitor.
