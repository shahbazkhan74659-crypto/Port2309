...
**Partially started ad hoc, 2026-08-27** (same pattern as Phase - X's real first blog post, written ahead of that phase's full completion): the first real project, "TS Library," was seeded via `projects/migrations/0011_seed_real_first_project.py`, which deleted all 3 Phase 3/5 placeholder `Project` rows outright (owner's explicit choice) rather than adding alongside them. TS Library has no screenshots yet — it renders via the `.mockup` fallback Phase Y built until real images are uploaded.

**Phase formally started, 2026-08-28**: a second real project, seeded under the working title "Entry Recorder" and same-day renamed to **LoadGate** (a single-user Django app the owner built for a manufacturing client, replacing a paper truck-loading logbook), was seeded via `projects/migrations/0012_seed_second_real_project.py` and renamed via `0014_entry_recorder_rename_loadgate.py` (slug now `loadgate`). Its GitHub repo (`RecordEntryApp`) was private and was switched to public (owner's explicit choice, after a check for committed secrets found none) so the portfolio's GitHub link resolves for visitors. Its `live_url` was then removed (`0013_entry_recorder_remove_live_url.py`) — the app is login-gated, so a public "Live" link had no value to a visitor; only the GitHub button remains. The owner chose to make LoadGate the new `featured=True` project (Home's featured card) — TS Library's row was updated to `featured=False`/`order=2` in the same seed migration, preserving the "at most one featured row" invariant. LoadGate's first screenshot (a real dashboard shot of the live client app, vetted to confirm no real client records or credentials were visible) was uploaded via the ORM shortly after, same day.

**Phase completed, 2026-08-29**: a third real project, **Fire Service Website**, was seeded via `projects/migrations/0015_seed_third_real_project.py` — a production Django + React/TypeScript + PostgreSQL business site with a custom Admin Hub CMS and lead-capture forms, built for a real freelance client, Iconic Techno Service (a fire & life-safety systems company), deployed live on Render and, per the owner, currently being migrated to a self-hosted Oracle Cloud VM to eliminate Render free-tier cold starts. The owner chose to make it the new `featured=True` project — LoadGate's row was updated to `featured=False`/`order=2` and TS Library shifted to `order=3` in the same migration, preserving the "at most one featured row" invariant. A real screenshot of the site's live Home page (owner-supplied) was uploaded as its first `ProjectImage` via the ORM, same convention as LoadGate's. Tags reused 5 existing `Tag` rows (Django, React, TypeScript, PostgreSQL, REST APIs) — no duplicates created. This closes out Phase 12's remaining open items: a third real project is seeded, and both previously-open projects (TS Library, LoadGate) already carried one real screenshot each from earlier sessions — so "screenshots for both existing ones" is also satisfied, not just "more real projects."

### Completion Criteria
Real `Project` rows exist with real content (not Phase 3/5 placeholder/mockup data) and each has at least one real screenshot (`ProjectImage`) rather than relying on the `.mockup` fallback indefinitely; the featured project (Home's card) reflects the owner's current choice.
...
### Scope
Owner-confirmed scope, 2026-09-03: build a real off-canvas mobile nav (the site had none — a repo-wide search confirmed this section's earlier "off-canvas/mobile nav behavior already present" phrasing was inaccurate speculation, not a verified fact) on both the public site and the Admin Hub, then audit and fix mobile-width (~390px) rendering across every page and component. First, a static design canvas prototype (mobile mockups of all 8 public pages, in the site's exact theme/tokens, plus a proposed nav-drawer design) was built and reviewed with the owner; this phase then implements that drawer for real and verifies the rest of the site against it.

### Completion Criteria
`manage.py check` runs clean; a working hamburger → slide-in drawer exists on both `templates/base.html` and `templates/adminhub/base.html`, listing every real destination (not just the 4-5 links in the desktop header nav), opens/closes via the burger, close button, backdrop click, and Escape, and correctly marks the current page; the existing Projects-detail and Resume-preview popup modals keep working unaffected on the same pages; desktop nav (`.nav-links`) is unaffected — no burger, no drawer, above the `640px` breakpoint; any genuine mobile-width overflow/collision found during a browser-verified audit pass is fixed.

**Status: Complete**, for the drawer + a first audit pass. Verified 2026-09-03 — `manage.py check` clean; `npm run build:css` clean; the drawer was verified live in a real narrow (390px) same-origin iframe render (`resize_window` did not affect the actual viewport in this session's browser environment, so a same-origin iframe was used as the practical narrow-viewport equivalent) on Home (open/close via burger, link click, close button all confirmed, `aria-current` accent-dot correct) and structurally on the Admin Hub via a direct authenticated template render (drawer markup, burger, all 7 hub URL names, and the re-skinned Log out row all present and resolving with no `NoReverseMatch`) — the Admin Hub's own interactive browser walkthrough was intentionally not done in-session (the login page had an autofilled saved password in the browser, and entering/submitting credentials on the user's behalf is out of bounds regardless of the field's own autofill) and is left for the owner to click through directly. The audit pass also caught and fixed one real pre-existing bug: `.resume-entry.hub-card`'s desktop-only `padding-right: 250px` (reserved for the absolutely-positioned Edit/Delete buttons) squeezed Admin Hub Resume entries into a ~100px content column at mobile widths — fixed with a `padding-right: 0; padding-top: 54px` override in the existing `640px` breakpoint. Not yet done: a full interactive click-through of every Admin Hub form page (`project_form.html`, `post_form.html`, etc.) at mobile width — static/CSS analysis found no other fixed-width hazards, but this wasn't visually confirmed in-browser this session.
...
**Status: Complete.** A second hosting option the owner found (`onehostmarket.com`, a budget cPanel reseller) was evaluated 2026-09-03 and set aside — no docs confirming Python/WSGI or PostgreSQL support, and a paid plan rather than free — so the plan stayed Render + Neon + UptimeRobot as originally decided. The codebase was made deploy-ready the same day: `gunicorn`, `whitenoise`, and media storage against **Cloudinary** (Render's free tier has no persistent disk, confirmed directly; Cloudflare R2 was tried first but requires a card on file to activate at all, confirmed live in Cloudflare's own dashboard, so this switched same-day to Cloudinary, whose free tier needs none), a dedicated Cloudinary API key, production security settings, and a `render.yaml` Blueprint. The Render web service (`shahbazkhan`, matching the owner's real name per the firstlast domain-naming convention most portfolios use) was then created from that Blueprint and went through three real, first-deploy-only bugs before coming up clean — see `DECISIONS.md`'s "First live Render deploy" entry for full detail: (1) `django-cloudinary-storage`'s own `collectstatic` override crashed on the legacy `STATICFILES_STORAGE` attribute this project never set; (2) with that fixed, the same override still ran and only partially collected Django admin's static files — fixed by reordering `INSTALLED_APPS` so Django's own `collectstatic` wins instead, since Cloudinary is only needed for media here; (3) `render.yaml`'s build command never ran `npm run build:css`, so the Tailwind stylesheet never existed on Render at all. A fourth, non-code gap was also found and fixed: Neon had zero `Resume`/`ProjectImage` rows — those were only ever uploaded via the ORM against the *local* Postgres dev database post-Phase-10, never against Neon — so a one-time script re-uploaded the real files to Cloudinary and created the matching rows directly against Neon. The live site was verified in-browser at `https://shahbazkhan.onrender.com` — styled correctly, real portrait/badge/project screenshots all loading from Cloudinary, `/projects/` rendering all three real projects. A free **UptimeRobot** monitor (`shahbazkhan.onrender.com`, HTTP, 5-minute interval, e-mail alerts) was then created and confirmed Up (100% uptime) — this satisfies Phase 15's completion criteria in full. The production superuser account (`Shahbaz`, created in Phase 10) had its password set to the owner's chosen production credentials, verified via a direct auth check.

## Phase 16 — AJAX Across the Site

### Objective
Apply AJAX (asynchronous, no-full-page-reload requests) site-wide so navigation and interactions feel smoother than the current full-page-reload-per-link behavior.

### Scope
Owner-confirmed complete 2026-09-03 at two delivered slices, rather than a further-expanding "every page" scope: the Projects list page's category/tag/search filtering and pagination/clear-filters (see `DECISIONS.md`'s "Projects page filter/search/pagination converted to AJAX" entry), and the Contact/Hire Me pages' form submissions (see `DECISIONS.md`'s "Contact/Hire Me forms converted to AJAX submission" entry) — both hand-rolled vanilla `fetch()` against the existing views (header-based content negotiation, no new dependency, no new URL), both with a hard constraint of no full page reload, and the Projects-page slice additionally with no browser URL change at all. This resolves the "vanilla `fetch()` vs. a library" and "how this reconciles with the React-islands-not-SPA boundary" open questions from this phase's original framing (`DECISIONS.md`'s "Production stack" decision) — vanilla, no new dependency, no client-side routing introduced, same pattern reused twice.

### Completion Criteria
Met for the scope actually delivered: the Projects page's search/tag/category filtering and pagination/clear-filters all update `#proj-results` via `fetch()` with zero full-page reloads and zero address-bar changes; the Contact/Hire Me forms submit via `fetch()` with zero full-page reloads; both pages' no-JS/direct-link/PRG fallbacks still work unchanged; the existing project-detail modal still opens correctly from AJAX-loaded results.

**Status: Complete**, at this scope. Owner's explicit call, 2026-09-03 — this phase is not gated on converting every remaining page/interaction to AJAX; the two slices built are the phase's full intended scope. Same pattern (header-based negotiation on the existing view, a matching `templates/partials/*` fragment, event delegation only where content actually gets swapped) is available to reuse if/when any further page ever wants the same treatment, but that would be new, separately-scoped work, not unfinished Phase 16 work.

## Phase 17 — SEO Optimization

### Objective
Improve the site's search-engine discoverability, shareability, and local search visibility.

### Scope
Owner-confirmed scope, 2026-09-04 (via direct questions — see `DECISIONS.md`): a real favicon, Open Graph/social meta tags, and `robots.txt` (all three were surveyed and explicitly excluded from Phase 9's scope — see that phase's entry above); per-page `<title>`/meta descriptions; a `sitemap.xml` covering every real page/project/post; `<link rel="canonical">` tags (relevant chiefly to the Projects page's filter/search/pagination query-param URLs); and structured data (JSON-LD, e.g. Person/CreativeWork schema). Also in scope: local-SEO keyword optimization — the owner wants the site to surface across a broad range of "freelancer/developer + location" search queries (e.g. "freelancers in Silvassa," "freelance developer around Daman," location terms including DNH, Vapi, Bhilad, Naroli) — the explicit goal is broad visibility across many such related queries rather than a #1 ranking on any single one. A codebase survey done at implementation time found per-page `<title>`/meta descriptions were already fully implemented (every page template already overrode `base.html`'s `{% block title %}`/`{% block description %}`) — so that specific item needed only a copy pass, not new plumbing; everything else in the scope above was genuinely missing and was built from scratch.

### Completion Criteria
`manage.py check`/`makemigrations --check` clean; `/robots.txt`, `/sitemap.xml`, and the favicon all resolve correctly; every page carries a correct canonical URL (filtered/paginated Projects URLs collapse to the clean `/projects/`), Open Graph/Twitter tags, and a site-wide JSON-LD `Person` block naming all six target locations; Project/Post detail pages show their own real title/description/image in their Open Graph tags rather than the site-wide default; at least one real, owner-approved sentence of local-SEO copy is live on the site.

**Status: Complete.** Implemented and verified 2026-09-04. Built: a favicon (`static/images/favicon.png`, the owner's own square badge image used as-is, no processing); a `core.views.robots_txt` view (`/robots.txt`, disallowing `/admin/`/`/adminhub/`, pointing at the live sitemap via `request.build_absolute_uri` so no domain is hardcoded); `core/sitemaps.py` (`django.contrib.sitemaps`, added to `INSTALLED_APPS` with no new dependency and no `django.contrib.sites`/`SITE_ID` needed — Django's sitemap view falls back to deriving the domain from the live request) covering every static route plus every real `Project`/`Post`; a new `core.context_processors.canonical_url` context processor (`request.build_absolute_uri(request.path)`, so query-string variants of a URL — Projects' filters/pagination — canonicalize to the clean path automatically); Open Graph + Twitter Card tags in `base.html` (Twitter tags deliberately omitted beyond `twitter:card`, since Twitter falls back to reading `og:*` tags when its own are absent); a site-wide JSON-LD `Person` schema (`templates/partials/schema_person.html`) with real `sameAs` (GitHub/LinkedIn) and a `workLocation` list covering Silvassa, Vapi, Daman, Dadra and Nagar Haveli, Bhilad, and Naroli — the clean, non-spammy mechanism for the local-search-visibility goal, in contrast to the keyword-stuffed `<title>` tags found on real local competitor sites surveyed during planning (Vaidik Infotech, Weblatic, TopWebLogic, WebsiteVale — see `DECISIONS.md`). One implementation-time discovery changed the original plan: Django does not allow the same `{% block %}` name to appear twice in one template (confirmed via a live `TemplateSyntaxError` — the plan had assumed it did), so Open Graph tags use their own `og_title`/`og_description`/`og_image` blocks (defaulting site-wide, explicitly overridden only on `project_detail.html`/`blog_detail.html` with the real project/post's own title/description/screenshot) rather than reusing `title`/`description` a second time. Real local-SEO copy (owner-confirmed wording, 2026-09-04 — see `DECISIONS.md`) was added to both `core.About.background_paragraph` (a new closing sentence) and `core.ContactEmail.available_for` (append location suffix) via data migration `core/migrations/0032_local_seo_copy.py`; About's and Hire Me's meta descriptions were also updated to mention the service area. Verified via `manage.py check` (clean), `makemigrations --check` (clean), and live `runserver`/`curl` checks: `/robots.txt` and `/sitemap.xml` both 200 with correct content (sitemap lists all 3 real projects and the real blog post); the favicon serves 200; a filtered/paginated Projects URL (`/projects/?category=Web&page=2`) confirmed canonicalizing to the clean `/projects/`; `project_detail.html` for "Fire Service Website" confirmed showing its own real title/description and its real uploaded screenshot as `og:image` (not the site-wide default); the JSON-LD block confirmed present and correct on the home page with all six target locations. **Deployed 2026-09-04** — `core/migrations/0032_local_seo_copy.py` applied to Neon automatically as part of `render.yaml`'s build-step `migrate` on the next push to `main` (see Phase 18's entry below and `DECISIONS.md`); confirmed live via `curl` against `https://shahbazkhan.onrender.com/about/`, showing the real local-SEO copy (Silvassa/Vapi/Dadra/Naroli/Bhilad all present).

## Phase 18 — Public Feedback / Testimonials Page

### Objective
Give clients a way to share feedback about working with the owner, on a dedicated public page.

### Scope
Owner-confirmed scope, 2026-09-04 (a locked design-canvas prototype first, then direct questions on the moderation mechanism — see `DECISIONS.md`). A public `/feedback/` page (matching the site's existing theme) with a name + 5-star rating + feedback-text submission form and a testimonial grid showing an average-rating summary — new submissions land as **pending** and are invisible on the public page until approved. A new `core.Feedback` model (`name`, `rating`, `feedback`, `status` — pending/live/viewed — `submitted_at`, `viewed_at`) backs both sides. Admin Hub gains a new "Feedback" page (`/adminhub/feedback/`) listing every pending and live entry with per-card actions: **Live** (pending → live, publishes it) — the same slot becomes **Unpublish** (live → pending, reverts it) once an entry is live; **Viewed** (pending or live → viewed — always un-publishes if it was live); and **Delete**. A separate Admin Hub page (`/adminhub/feedback/viewed/`, reached via a "View Viewed Feedback →" button on the main queue page, not present anywhere on the public site) lists every viewed entry as a literal table — same database table, a different view — and viewed entries auto-delete 20 days after being marked Viewed via a lazy delete-on-request sweep (no scheduler/cron infrastructure exists on this project's host). "Feedback" was added as the last link in both the public desktop nav and the mobile drawer (and the equivalent Admin Hub nav), and deliberately **not** added to the public footer, per the owner's explicit correction on the locked design prototype.

### Completion Criteria
`manage.py check`/`makemigrations --check` run clean; `/feedback/` renders the form and only ever shows `status="live"` testimonials, with a correct average-rating summary; a submission is saved as `pending` and stays invisible on the public page until an admin acts on it; `/adminhub/feedback/` lists pending/live entries with working Live/Unpublish/Viewed/Delete actions, each reflected immediately on the public page; `/adminhub/feedback/viewed/` lists viewed entries in a table with a working delete, and an entry whose `viewed_at` is more than 20 days old is swept (deleted) the next time that page loads; nav placement matches the locked design (last link, public + hub, excluded from the public footer); Contact/Hire Me's existing AJAX/PRG behavior is unaffected by the shared `_handle_lead_form` helper's extension.

**Status: Complete**, for this scope. Implemented and verified 2026-09-04 — see `DECISIONS.md`'s "Phase 18 Public Feedback" entry for the full implementation detail and verification steps. Two follow-up refinements shipped the same day: the "See My Resume" button was removed from the Feedback page's header (owner's request — the page reverted to the plain single-column `.page-head` shape `blog.html` already uses, rather than the two-column form used by Contact/Hire Me/About/Projects), and the star picker gained a responsive rating word (Poor/Fair/Good/Great/Excellent, matching the original design-canvas prototype) via a CSS-only `:has()`-based reveal — see `DECISIONS.md`. The Admin Hub Feedback queue's Live/Unpublish/Viewed/Delete actions were also converted to AJAX (no page reload), extending the site's existing header-negotiation pattern to action endpoints for the first time — see `DECISIONS.md`'s "Admin Hub Feedback queue converted to AJAX" entry. **Deployed 2026-09-04** — `core/migrations/0033_feedback.py` applied to Neon automatically as part of `render.yaml`'s build-step `migrate` when this work was pushed to `main`; confirmed live via `curl` against `https://shahbazkhan.onrender.com/feedback/` (200, correct markup, an empty testimonial grid as expected — Neon's `Feedback` table has no rows yet, since none were seeded and the local dev submissions never sync there, per the standing "local dev and Neon are independent databases" note in `TASKS.md`). **Deliberately excluded from search:** the owner does not want the Feedback page surfacing as a search result — `/feedback/` is left out of `/sitemap.xml`, and (since a sitemap omission alone doesn't stop a crawler discovering a page linked from every page's nav) the page also carries `<meta name="robots" content="noindex, follow">`; see `DECISIONS.md`.

Phase 18 is now the last *numbered* phase in the roadmap. Phase 15 was confirmed as the last phase 2026-08-24/2026-09-03 (see below); the project owner does not expect further phases beyond Phase 16 unless new ones are identified (originally confirmed 2026-08-24 for what was then Phase 12; six additional phases were inserted later: Phase 11 "Seeding Real Projects" (later renumbered to Phase 12), Phase 11 "Custom Admin Hub", the non-numeric **Phase - X** "Blog Upgrade: Content, Logic, and Tables" and **Phase Y** "Project Page Upgrade: Logic and Content-Ready Interface" (all three 2026-08-25), **Phase 13 "Mobile Responsive Behaviour"** (2026-08-29, an explicit renumbering rather than a non-numeric label — see the note at the top of this file), and **Phase 16 "AJAX Across the Site"** (2026-09-03). Phase - X and Phase Y sit after Phase 10 and before Phase 11 in build order (in that sequence — X then Y) despite their labels not being part of the numeric sequence. Two more phases, **Phase 17 "SEO Optimization"** and **Phase 18 "Public Feedback / Testimonials Page"**, were added 2026-09-04 (owner's request; scope for Phase 17 and the decision to split Phase 18 out separately were both confirmed via direct questions — see `DECISIONS.md`) — appended after Phase 16 rather than inserted mid-sequence, so no existing phase numbers changed. If new phases are identified later, add them here rather than assuming the roadmap is fixed forever.
