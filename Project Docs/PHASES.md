...
### Scope
Still open overall — which further pages/interactions convert, if any, remains to be defined by the project owner. The first concrete slice is now settled and built (2026-09-03, see `DECISIONS.md`'s "Projects page filter/search/pagination converted to AJAX" entry): the Projects list page's category/tag/search filtering and its pagination/clear-filters now go through vanilla `fetch()` calls against the existing `project_list` view, with a hard, owner-specified constraint — no full page reload AND no browser URL change at all (no querystring in the address bar, no `history.pushState`/`replaceState`). This resolves the "vanilla `fetch()` vs. a library" question for at least this slice (vanilla, no new dependency) and reconciles cleanly with the standing React-islands-not-SPA boundary (`DECISIONS.md`'s "Production stack" decision) since no client-side routing is introduced — only a same-page content swap.

### Completion Criteria
To be defined for the phase as a whole. For the Projects-page slice specifically: search/tag/category filtering and pagination/clear-filters all update `#proj-results` via `fetch()` with zero full-page reloads and zero address-bar changes; the existing no-JS/direct-link fallback (`?q=...&category=...&tag=...&page=...`) still server-renders correctly; the existing project-detail modal still opens correctly from AJAX-loaded results.

**Status: In progress.** The Projects page's filter/search/pagination is done (see `DECISIONS.md`) — everything else under this phase (other pages/interactions, if any) remains unscoped.
