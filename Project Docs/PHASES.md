...
### Scope
To be defined in further detail by the project owner. Identified 2026-09-03, after Phase 15 (deployment) closed out — not yet scoped: which pages/interactions convert first, whether this uses vanilla `fetch()`/`history.pushState` or a small library, and how it interacts with the existing server-rendered-Django-templates architecture (see `DECISIONS.md`'s "Production stack" decision — React was always scoped to isolated islands, not full-page SPA routing; this phase's approach needs to be reconciled with that boundary before implementation starts).
