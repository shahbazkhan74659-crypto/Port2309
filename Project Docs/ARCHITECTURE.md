...
**Planned:** None remaining for `projects`' core CRUD surface — future work is data-only (Phase 12). Real blog content authoring remains open (Phase - X's own scope, not yet complete — see `PHASES.md`).
...
**Planned:** Real resume experience/education content and the actual resume PDF file remain deferred — no phase scheduled yet. Real (non-placeholder) `Project` content is Phase 12, once the Phase 10 production database is in place. Additional blog posts beyond this first one are not scoped to any phase — they can be authored anytime via the admin.
...
- `Post.tags`' 1–6 count rule is enforced only by `PostAdminForm.clean_tags()` in `projects/admin.py`, not by the database or `Model.full_clean()` (a `ManyToManyField`'s cardinality can't be validated before the instance has a PK). Any future code path that creates/edits `Post.tags` outside the Django admin — a script, `manage.py shell`, a future public submission form — will not get this validation automatically; it would need to call `clean_tags()`'s logic itself or reuse `PostAdminForm`. `Project.tags`/`About`'s skill M2M fields have no upper-bound count constraint at all, only Django's default "at least 1" (no `blank=True` set).
