...
- **This repository currently contains a single static prototype file** (`prototype/index.html`) and no production application code, no `package.json`, and no Django project. The repo is git-initialized and pushed to `origin` (`https://github.com/shahbazkhan74659-crypto/Port2309.git`, branch `main`) as of 2026-08-24. Do not assume any backend, framework, or build tooling exists — verify against the filesystem before making claims.
- A production tech stack (Django + Django templates + Tailwind CSS + React/JS + a relational database, engine TBD) and Django app boundaries were decided during prior planning discussions. See `DECISIONS.md`. **Do not scaffold the Django project or start production implementation until the project owner explicitly asks for it** — this was an explicit instruction from the owner during planning.
